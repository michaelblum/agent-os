...
func buildCompiledCommandRegistry() -> [CommandDescriptor] {
    ...
    reg.append(CommandDescriptor(path: ["dev"], summary: "Repo development workflow router and build surface", forms: [
        ...
        InvocationForm(id: "dev-afk-session-trigger", usage: "aos dev afk-session-trigger (--packet <packet.json>|--afk-work-queue <queue.json>) (--dry-run|--supervised-live-launch --i-am-present --json|--afk-live-launch --afk-authorization <authorization.json> --json --out <receipt.json>|--warm-dock-tui-reuse --json) [--queue-run-fixture <fixture.json>] [--afk-authorization <authorization.json>] [--sleep-lease <lease.json>] [--sleep-lease-live-launch] [--provider <name>] [--dock <dock>] [--repo <path>] [--timestamp <iso>] [--out <path>] [--result-route <ref>] [--idempotence-salt <value>] [--existing-receipt <path>] [--replacement-for <id>] [--bridge-visibility-fixture <path>] [--cleanup-proof-fixture <path>] [--provider-session-id <id>] [--launch-observed-at <iso>] [--codex-home-fixture <path>|--codex-home <path>]",
            args: [
                flag("packet", "--packet", "Manual transfer packet JSON path for the experimental prototype session-trigger diagnostic"),
                flag("afk-work-queue", "--afk-work-queue", "Local AFK work queue JSON path for dry-run, fixture-run, or one-item live launch validation"),
                flag("queue-run-fixture", "--queue-run-fixture", "Local queue fixture JSON path for deterministic AFK work queue fixture runs"),
                flag("dry-run", "--dry-run", "Dry-run guard; no provider, terminal, gateway, or route is launched", type: .bool),
                flag("afk-authorization", "--afk-authorization", "Local AFK authorization JSON path for approved AFK runs"),
                flag("sleep-lease", "--sleep-lease", "Compatibility alias for --afk-authorization"),
                ...
                flag("afk-live-launch", "--afk-live-launch", "Experimental AFK live launch intent; requires --afk-authorization --json --out and forbids --i-am-present", type: .bool),
                flag("sleep-lease-live-launch", "--sleep-lease-live-launch", "Compatibility alias for --afk-live-launch", type: .bool),
                ...
                flag("provider-launch-dry-run", "--provider-launch-dry-run", "Fixture-only provider launch attempt diagnostic; forbidden with AFK authorizations", type: .bool),
            ...
            examples: [
                ...
                "aos dev afk-session-trigger --packet /tmp/manual-afk-packet.json --provider codex --dock gdi --afk-live-launch --afk-authorization /tmp/authorization.json --json --out /tmp/receipt.json",
                "aos dev afk-session-trigger --afk-work-queue /tmp/queue.json --provider codex --dock gdi --afk-live-launch --afk-authorization /tmp/authorization.json --json --out /tmp/receipt.json"
            ]),
    ...
    reg.append(CommandDescriptor(path: ["permissions"], summary: "Permission preflight and one-time onboarding", forms: [
        ...
        InvocationForm(id: "permissions-reset-runtime", usage: "aos permissions reset-runtime [--mode repo|installed] [--allow-service-reset --emergency-ack-other-apps] [--dry-run] [--json]",
            ...
            stdin: nil, constraints: nil,
            execution: execMutating(daemon: false),
            output: outJSONFlag,
            examples: ["aos permissions reset-runtime --mode repo", "aos permissions reset-runtime --mode repo --dry-run --json"])
    ]))
