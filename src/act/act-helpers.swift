// helpers.swift — Key codes, arg parsing, math utilities for the act module
// AX helpers (axString, axChildren, axActions, axBounds) live in perceive/ax.swift.
// Error output (exitError) lives in shared/helpers.swift.

import ApplicationServices
import CoreGraphics
import Foundation

// MARK: - JSON Output (ndjson line writer for session mode)

private let actJsonEncoder: JSONEncoder = {
    let enc = JSONEncoder()
    enc.outputFormatting = [.sortedKeys]
    return enc
}()

func writeJSONLine<T: Encodable>(_ value: T, to handle: FileHandle = .standardOutput) {
    guard let data = try? actJsonEncoder.encode(value),
