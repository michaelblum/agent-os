...
- `src/perceive/image-file-compare.swift` owns the stateless, daemon-free PNG
  file comparator behind private `__see compare`. It decodes one upright,
  non-animated PNG per input into bounded canonical sRGB premultiplied RGBA,
  compares identical decoded geometry, and emits stable JSON without capture,
  interactive preflight, permissions, polling, or native runtime lifecycle;
