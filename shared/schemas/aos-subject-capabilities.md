...
| `replayable` | The Subject has execution knowledge that a caller can re-run: a Workflow, source-backed Recipe, repairable Work Record execution map, or transitional Step Descriptor. Consumers check origin, preconditions, compatibility vocabulary, and available tools before replay. Current Gate fields on transitional descriptors are legacy coupling and do not grant AOS permission; Gate remains optional caller input under ADR 0040. |
