...
| `replayable` | The Subject advertises a current source-backed executable contract that a caller can invoke, such as a Workflow or source-backed Recipe. A Work Record execution map, repair hint, or Step Descriptor is neutral evidence/descriptive input and does not by itself make a Subject replayable. Consumers mechanically check the advertised execution contract, exact source identity, preconditions, compatibility vocabulary, and available tools before invocation; Gate remains optional caller input under ADR 0040. |
