...
`snapshot_scoped` click and fill refs run a fresh xray validation before any
real mutation; the saved ref, role, label/title, and context must still match.
AOS canvas `reacquirable` click and set-value refs may route through the
current canvas resolver. Native AX `volatile` refs are inspection-only.
