...
| `type`, `key` | command-specific `aos do ... ref:<...>` | none | none | command-specific | no supported mutation | varies; can silently affect the wrong surface | public `ref:` targets route to the saved-ref resolver before backend wrappers and fail closed for unsupported actions | `REF_NOT_FOUND`, `REF_AMBIGUOUS`, `ACTION_INCOMPATIBLE`, or `REF_UNSUPPORTED` | recommended next command is a fresh saved capture | browser `type`/`key` need a separate saved-ref grammar decision before mutation support can be added |
