...
`snapshot_scoped` click, fill, hover, scroll, and drag refs run a fresh xray
validation before any real action; the saved ref, role, label/title, and
context must still match. AOS canvas `reacquirable` click and set-value refs
may route through the current canvas resolver. Native AX `volatile` refs are
inspection-only and explicitly report that no saved-action no-foreground
guarantee is made.
...
| `fill` | `aos do fill ref:<snapshot-id>:<ref> <text> --workspace <id>` | browser | `snapshot_scoped` | browser textbox/searchbox/combobox/input ref plus text | yes | high; edits browser field state | load saved ref, require unambiguous scope, require supported action, validate current browser xray ref, role, title/label, context, and enabled state, then dispatch existing browser fill | `dry_run`, `success`, `REF_STALE`, `REF_UNSUPPORTED`, `ACTION_INCOMPATIBLE`, `REF_AMBIGUOUS`, `REF_REVALIDATION_FAILED` | action response includes Playwright execution metadata and carried `state_id`; agents should refresh capture after mutation before using more refs | validation does not prove semantic intent after navigation or DOM replacement; stale, missing, ambiguous, disabled, or changed targets fail closed |
| `hover` | `aos do hover ref:<snapshot-id>:<ref> --workspace <id>` | browser | `snapshot_scoped` | ref target | yes | low-to-medium; can reveal hover state | load saved ref, require unambiguous scope, require supported action, validate current browser xray ref, role, title/label, context, and enabled state, then dispatch existing browser hover | `dry_run`, `success`, `REF_STALE`, `REF_UNSUPPORTED`, `ACTION_INCOMPATIBLE`, `REF_AMBIGUOUS`, `REF_REVALIDATION_FAILED` | action response includes Playwright execution metadata and carried `state_id`; agents should refresh capture if hover reveals new UI | validation is xray/ref based and fails closed after navigation, DOM replacement, disabled targets, or role/title/label/context drift |
| `scroll` | `aos do scroll ref:<snapshot-id>:<ref> <dx,dy> --workspace <id>` | browser | `snapshot_scoped` | ref target plus `dx,dy` delta | yes | medium; can change viewport/state | load saved ref, require unambiguous scope, require supported action, validate current browser xray ref, role, title/label, context, and enabled state, then dispatch existing browser scroll | `dry_run`, `success`, `REF_STALE`, `REF_UNSUPPORTED`, `ACTION_INCOMPATIBLE`, `REF_AMBIGUOUS`, `REF_REVALIDATION_FAILED`, `MISSING_ARG`, `INVALID_ARG` | action response includes Playwright execution metadata and carried `state_id`; agents should refresh capture after scrolling before choosing more refs | browser scroll uses direct adapter `dx,dy` semantics and does not prove semantic intent after navigation or DOM replacement |
| `drag` | `aos do drag ref:<snapshot-id>:<from-ref> ref:<snapshot-id>:<to-ref> --workspace <id>` | browser | `snapshot_scoped` | two browser refs from the same saved snapshot and browser session | yes | medium; can mutate page state through pointer drag | load both saved refs, require unambiguous scope, require both support `drag`, require same snapshot and browser session, validate both current browser xray refs, roles, title/labels, contexts, and enabled states, then dispatch existing browser drag | `dry_run`, `success`, `REF_STALE`, `REF_UNSUPPORTED`, `ACTION_INCOMPATIBLE`, `REF_AMBIGUOUS`, `REF_REVALIDATION_FAILED`, `MISSING_ARG`, `INVALID_REF_TARGET` | action response includes Playwright execution metadata and carried `state_id`; dry-run includes primary and secondary current validation | browser drag is target-to-target Playwright drag; validation does not prove semantic intent after navigation or DOM replacement, and stale, missing, ambiguous, disabled, changed, cross-snapshot, or cross-session endpoints fail closed |
