...
export const LEGACY_MOUNTED_SURFACE_MENU_QUERY_PARAM = 'aos_manifest_menu';
export const MOUNTED_SURFACE_MENU_PROJECTION_SCHEMA_VERSION = 'aos.mounted-surface-menu-projection.v0';

export function mountedSurfaceMenuItemsForSurface(menu = [], surfaceId = '') {
  return (Array.isArray(menu) ? menu : []).filter((item) => item?.surface === surfaceId);
}
