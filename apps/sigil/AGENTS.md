...
| `renderer/live-modules/voice-runtime.js` | Sigil voice orchestration adapter. It wires toolkit-backed dictation, toolkit-normalized `inputIdentity`, Sigil response policy, live renderer state, interaction trace, status-menu backend selection, and render scheduling without re-normalizing transport identity or putting those guts back into `main.js`. |
