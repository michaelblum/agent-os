...
| `renderer/live-modules/avatar-parking.js` | Sigil avatar parking controller for terminal/status collapse. Keep parking point math and restore bookkeeping here; keep terminal lifecycle decisions and host canvas mutations in `main.js` until they have a narrower owner. |
...
| `renderer/live-modules/utility-canvas-config.js` | Sigil utility canvas ids, content URLs, and default frame geometry. Keep host canvas lifecycle and live state mutation in `main.js`; keep reusable frame/config math here with injected display bounds. |
