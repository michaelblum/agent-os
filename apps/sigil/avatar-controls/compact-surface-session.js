...
function requiredFunction(name, value) {
    if (typeof value !== 'function') {
        throw new TypeError(`Sigil avatar controls compact surface session requires ${name}.`);
    }
    return value;
}

export function createAvatarControlsCompactSurfaceSession({
    anchor,
    state,
    document,
    visualObjectBinding,
    ...
    if (!anchor) throw new TypeError('Sigil avatar controls compact surface session requires anchor.');
    if (!document) throw new TypeError('Sigil avatar controls compact surface session requires document.');
    if (!visualObjectBinding?.routeHandlers || !visualObjectBinding?.rendererSyncHandlers) {
        throw new TypeError('Sigil avatar controls compact surface session requires visualObjectBinding handlers.');
    }
