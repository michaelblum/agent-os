...
- Migration caveat — some current helpers/tests emit app-specialized `subject_type` values (e.g. `sigil.agent`) directly from wiki paths. That is legacy projection behavior to unwind, not the target model. Pending: code/schema audit to identify and replace those call sites.
- `capabilities[]` today contains operation/event-contract strings like `markdown_document.text.patch`, `wiki.invoke`, `work_record.execution_map.edit`, `canvas_object.effects.patch`. These are useful but they are *operation contracts*, not high-level Capability classes (per ADR-0010). Two paths forward, deferred to schema design: keep both inside `capabilities[]` and document the shape, or split into `capabilities[]` (high-level: `inspectable`, `editable`, etc.) plus `contracts[]` (dotted operation strings). The plan's Phase 4 line 121 ("opening editor facets by subject capability") should read as: "the Subject Browser checks `capabilities` for `editable`; when present, surfaces the matching controls/editor-Layer Facets as openable editors."
- "subject chain" (used in `aos-grand-unification-plan.md` Phase 3) — resolved: this is a **Navigation Trail** of Subject Entry Handles, not a chain of Subjects. The plan wording should be updated when the schema work lands.
- Work Record `origin` field shape — **resolved (ADR-0009)**: `origin: { kind, ref }` where `kind ∈ ad_hoc | recipe | playbook | workflow`. Documentation-only Recipes are *not* origins; they are cited via a separate `references[]` array with `relationship: "guided_by"`. Pending: schema design pass for `references[]` shape.
...
- Verifier Report shape — the plan lists `claims`, `verified`, `failed`, `unverified` as four parallel fields. Resolved direction (ADR-0003): use `claim_results[]` as the source of truth; if the four parallel fields persist, they are *derived indexes of Claim IDs*, not independent storage. When a Verifier Report is embedded in a Work Record it should not echo the full `claims` list (single source of truth); when reports travel standalone, they include a `claims_digest` for auditability.
- A Playbook needs explicit syntax to *promote* a step Postcondition into a Work Record Claim (Playbooks should be able to declare run-wide outcomes, not only step-local gates). Pending: Playbook step grammar design.
- `--anchor-browser` (and sibling `--anchor-window`, `--anchor-channel`) reads as a parallel target dialect in `aos-grand-unification-plan.md` Phase 1, but it is a *role flag* whose value is a regular Target-with-Ref. Plan wording should clarify; longer-term a generic `--anchor <target>` flag may consolidate them, but that is a future cleanup, not a plan rewrite. See ADR-0004.
- `facets[].host` enum (`"browser" | "canvas" | "either"`) was considered and rejected as too coarse — a Facet may have *multiple Host implementations* with different entry points, target dialects, or fidelity. Resolved direction: `facets[].hosts[]` array of `{ kind, target_dialect, entry, ... }` records, with optional preference ordering. Pending: schema design pass.
- "Dual-hosting" (used in `aos-grand-unification-plan.md` Phase 4) — resolved meaning: shipping a Facet with both Browser-Host and Canvas-Host implementations. The plan's "do not require dual-hosting for every editor" should be read as "Facets may declare a single Host; a Browser-Host implementation alone is fine for most editor/workbench Facets."
