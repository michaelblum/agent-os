...
- `capabilities[]` today contains operation/event-contract strings like `markdown_document.text.patch`, `wiki.invoke`, `work_record.execution_map.edit`, `canvas_object.effects.patch`. These are useful but they are *operation contracts*, not high-level Capability classes (per ADR-0010). The v-next schema sketch recommends splitting these into `contracts[]` while keeping a migration period where consumers read both locations. The plan now says the Subject Browser opens controls/editor-Layer Facets when the Subject advertises `editable`.
