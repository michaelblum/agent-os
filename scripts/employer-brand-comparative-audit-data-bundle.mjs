...
const defaultOut = 'docs/design/fixtures/aos-artifacts/employer-brand-comparative-audit/data-bundle.json';

function usage() {
  return `Usage: node scripts/employer-brand-comparative-audit-data-bundle.mjs [--fixture-root <dir>] [--out <file>]

Builds the normalized Employer Brand Comparative Audit Data Bundle V0 from
checked-in fixture inputs. This is deterministic data shaping only; it does not
render reports, browse websites, collect evidence, run workflows, or export files.`;
}
...
function main() {
  ...
  if (!validation.valid) {
    throw new Error(`Data bundle validation failed: ${validation.errors.join('; ')}`);
  }
