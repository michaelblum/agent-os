...
function usage() {
  return `Usage: node scripts/employer-brand-repaired-live-evidence-element-clip-verify.mjs [--fixture-root <dir>] [--manifest <file>] [--json]

Verifies the repaired live evidence capture manifest against the repaired run
scope: 4 attempted repaired slots, LinkedIn source-unavailable preserved, 14
other context entries preserved, full_page_grab=false, and non-goal controls off.`;
}
...
function main() {
  ...
  const result = verifyEmployerBrandRepairedLiveEvidenceElementClipManifestObjective(readJson(manifestPath), {
    fixtureRoot,
  });
  if (args.json) {
    console.log(JSON.stringify(result, null, 2));
  } else {
    console.log(`repaired live evidence element clip verification: ${result.status}`);
