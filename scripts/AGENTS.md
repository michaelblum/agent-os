...
- `aos-see-native.mjs` owns native perception admission, while
  `lib/aos-see-child-runner.mjs` guards the exact native child for both direct
  and saved captures. Parent or wrapper loss must reach that child, escalate
  after a bounded grace period, and never leave a capture process orphaned.
