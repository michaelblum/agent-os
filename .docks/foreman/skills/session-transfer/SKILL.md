...
| GDI round | `docs/design/work-cards/<card>.md` for non-trivial implementation or validation contracts. | Select `gdi` with structured `agent_type=gdi` when available. If structured `agent_type` is unavailable, stop with a subagent-runtime blocker instead of using prompt-prefix fallback. |
