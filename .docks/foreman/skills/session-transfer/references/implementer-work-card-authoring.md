# Implementer Work Card Authoring

Use this reference when Foreman writes or revises a work card for a fresh Implementer
session. The goal is reliable context transfer without freezing one permanent
prompt format. Implementer starts from a new context window, so the work card must carry
...
- Any draft evidence commit or branch that Implementer must classify as retain, amend,
  supersede, or revert.

### Fresh Context Contract

State that Implementer must not rely on parent-thread memory:
...
List the smallest durable files Implementer should read before searching:
...
Do not create a universal fixed read list for every Implementer task. Use a reusable
...
Do not tell Implementer to stop merely because `pgrep` or `ps` finds
...
- unmanaged owner: service/launchd facts do not explain the socket owner, or the
  ...
  `manual_intervention` and report PID, PPID, command line, socket owner, and service
  status evidence.

Under `local single-checkout workflow`, Implementer must work from the single checkout at
`/Users/Michael/Code/agent-os`. Do not tell Implementer to create linked git worktrees;
...
or inactive input-tap blockers, include the deterministic Implementer stop branch:

```bash
the manual TCC blocker report path
```

The Implementer helper prints the manual-intervention TCC blocker and does not write hook
markers, reset permissions, open Settings, or start AOS. Implementer must stop with
`manual_intervention` instead of retrying live checks or running permission setup
commands. After the human returns with `finished`, Implementer returns the blocker to
Foreman for the next controlled step.

### Existing Code To Inspect

Name concrete files and why they matter. This is often the highest-leverage
section in a fresh Implementer session.
...
This helps Implementer keep implementation at the right layer while still allowing it to
adjust after reading code.

### Hard Boundaries / Non-Goals

Use explicit anti-scope language. Strong prior Implementer work cards were good because
they told Implementer what not to resume:
...
Offer likely files/modules, but label them as suggestions. Implementer should inspect
...
For non-trivial work, especially in a dirty checkout, ask Implementer for a lightweight
...
### Post-Implementer Review Option

For non-trivial implementation cards, Foreman may plan a separate bounded review
after completion:

- do not make review part of Implementer's assigned implementation task unless the user
  explicitly asks;
- after Implementer reports completion, Foreman inspects local diff/status/evidence;
...
- Foreman may send a follow-up or correction prompt for the same Implementer session
  when the context is still useful and the fix is narrow;
- let Implementer address only tiny mechanical review fixes directly when no acceptance
  judgment is needed.
...
Use when Implementer prepares work for Operator:
...
Before handing to Implementer, Foreman should be able to answer:

- Could Implementer start from a blank context and find the right files?
...
- Is the verification concrete enough that Implementer cannot hand-wave success?
