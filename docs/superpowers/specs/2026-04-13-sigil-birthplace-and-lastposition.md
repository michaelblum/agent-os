...
```
daemon.setLastPosition(agent_id, { x, y })
```
...
- `agent.lastPosition.get { agent_id } → { agent_id, position: {x,y} | null }` — request/response.
- `agent.lastPosition.set { agent_id, x, y }` — fire-and-forget.

Exact wire names are refinable at plan time (`sigil.*` vs `agent.*` vs generic state KV). The spec requires only:
- a request/response read keyed by `agent_id`, returning the current position or null;
- a fire-and-forget write keyed by `agent_id` with `{x, y}`;
