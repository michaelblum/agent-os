...
The current unpublished M3D-V1 candidate preserves four exact track selections, one
...
follow-deadline truth without live native authority.
...
M3A/M3B/M3C-V2/M3D-V1 fixed display/window/region and caller-followed region
...
5. M2 adapts the existing microphone owner to authenticated peer identity and
   ...
   M3D-V1 adds caller-followed region geometry with generation-CAS updates;
