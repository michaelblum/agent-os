...
| `aos ops` command surface | deleted | `manifests/commands/source/`, canonical `aos recipe` manifests, `scripts/aos-ops.mjs` implementation | `bash tests/help-contract.sh`; `bash tests/external-command-dispatch.sh`; negative assertions: `./aos help ops --json` and `./aos ops list --json` return `UNKNOWN_COMMAND` |
