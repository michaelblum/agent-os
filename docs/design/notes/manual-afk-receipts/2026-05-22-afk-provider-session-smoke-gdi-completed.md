# Manual AFK Receipt: provider-session-smoke GDI completed

receipt_bundle_id: manual-afk-2026-05-22-provider-session-smoke-gdi
status: completed
...
- selected_action: dry-run preflight followed by supervised provider-session
  smoke
- idempotence_key:
  `manual-afk-provider-session-smoke:gdi:codex:9d0ec0d7`
...
- dispatch_attempt_id: `manual-codex-gdi-smoke-2026-05-22`
- preflight_command:
  `./aos dev afk-dry-run --packet <packet.json> --provider codex --dock gdi --json --timestamp 2026-05-22T01:00:00.000Z`
...
- provider_facts:
  ...
  - model `gpt-5.5 low`
  - YOLO permissions
  - terminal/status branch `gdi/afk-dev-dry-run-command-v0`
- provider_session_id: `019e4e18-74f7-74c1-86af-53a7c4962b7a`
- provider_reported_branch: `gdi/afk-dev-dry-run-command-v0`
...
- commands_checks:
  ...
  - `./aos ready`
  - `./aos dev afk-dry-run --packet <packet.json> --provider codex --dock gdi --json --timestamp 2026-05-22T01:00:00.000Z`
  - supervised no-op GDI validation dispatch
- no_mutation_claims:
...
- before_status: `## gdi/afk-dev-dry-run-command-v0`
- after_status: `## gdi/afk-dev-dry-run-command-v0`
- after_head_short: `9d0ec0d7`
- ready_output: `ready=true mode=repo daemon=reachable tap=active`
- dry_run_result: exit 0; final_status completed; provider codex; dock gdi;
  launch_root `.docks/gdi`; launch_performed false; all 7 validations passed;
  artifacts_created empty
- supervised_gdi_result: launched from `.docks/gdi`, reported expected branch,
