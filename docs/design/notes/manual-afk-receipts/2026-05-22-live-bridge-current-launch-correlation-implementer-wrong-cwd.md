# Manual AFK Receipt: live bridge current-launch correlation Implementer wrong cwd

receipt_bundle_id: manual-afk-2026-05-22-live-bridge-current-launch-correlation-implementer
...
- branch: `implementer/afk-bridge-current-launch-observability-correction-v0`
...
- idempotence_key:
  `manual-afk-live-bridge-current-launch:implementer:codex:72fb1cfa`
...
- bridge_command:
  `SIGIL_AGENT_TERMINAL_PORT=17863 SIGIL_AGENT_TMUX_SESSION=afk-provider-correlation SIGIL_AGENT_CWD=/Users/Michael/Code/agent-os/the implementer native subagent SIGIL_AGENT_COMMAND='codex --no-alt-screen' SIGIL_AGENT_TERMINAL_DRIVER=process node apps/sigil/codex-terminal/server.mjs`
...
- provider_session_meta_cwd: `/Users/Michael/Code/agent-os/the operator native subagent`
...
- provider_visible_title:
  ...
  - branch: `implementer/afk-bridge-current-launch...`
  - model: `gpt-5.5`
...
- goal: launch one supervised Codex/Implementer provider session through the existing
...
- provider_owned_artifacts: Codex transcript files were created by the
  ...
  `/Users/Michael/Code/agent-os/the operator native subagent`, not the requested
...
- no_mutation_claims:
  ...
  - no PR
- next_owner: foreman
- follow_up: route a focused deterministic Implementer correction for observed
...
- missing_evidence:
  ...
  - classifier did not receive the observed provider session id because the
    session was outside the requested Implementer cwd-filtered catalog payload

## Readiness Implication

The live smoke validates the accepted fixture-backed classifier for the
requested Implementer cwd filter: it correctly refused to bind stale Implementer catalog
...
`the operator native subagent` as cwd, while the bridge launch intent requested `the implementer native subagent`.
