# Manual AFK Receipt: bridge-backed provider launch GDI partial

receipt_bundle_id: manual-afk-2026-05-22-bridge-backed-provider-launch-gdi
status: partial_pass
...
- branch: `gdi/afk-launch-attempt-prototype-no-provider-v0`
...
- idempotence_key:
  `manual-afk-bridge-provider-launch:gdi:codex:81af5f0e`
...
- requested_bridge_command:
  `PORT=17861 SESSION=afk-provider-smoke BRIDGE_SESSION=afk-provider-smoke-bridge CANVAS_ID=afk-provider-smoke-terminal CWD_TARGET=/Users/Michael/Code/agent-os/.docks/gdi apps/sigil/agent-terminal/launch.sh --new-codex --restart`
...
- provider_launch_cwd: `/Users/Michael/Code/agent-os/.docks/gdi`
- provider_facts:
  - Codex CLI `codex-cli 0.133.0`
  - terminal title exposed cwd `.docks/gdi`
  - terminal title exposed branch
    `gdi/afk-launch-attempt-protot...`
...
- goal: launch Codex through the existing Sigil terminal bridge from
  `.docks/gdi`, collect bridge/catalog/telemetry evidence, and confirm no
...
- no_mutation_claims:
  ...
  - no PR
- next_owner: foreman
- follow_up: route a focused local GDI correction/observability slice for
  current-launch catalog/telemetry correlation before implementation of a
  supervised real-launch attempt record

## Evidence Receipts

- before_status: `## gdi/afk-launch-attempt-prototype-no-provider-v0`
- after_status: `## gdi/afk-launch-attempt-prototype-no-provider-v0`
...
- provider_visible_result: Codex launched visibly through bridge from
  `.docks/gdi`; version, cwd, branch, model, HEAD, and clean status were visible
- missing_evidence:
...
provider session from the GDI dock root without mutating local or external
