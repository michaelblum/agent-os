# GDI Work Card: Live Input And Rendering Regression Repair V0
...
- `branch_from`: current workspace branch
  `gdi/sigil-avatar-panel-final-frame-avoidance-assessment-v0`
- `minimum_code_start_ref`: `1cb60a5801960aca0c3120236b7b9549fcdffbf4`
- `required_start_ref`: current dirty workspace on
  `gdi/sigil-avatar-panel-final-frame-avoidance-assessment-v0`
- `expected_output_branch`: keep the current branch unless GDI must create a
...
Then stop with `human_needed`. Do not run permission setup, TCC reset, or
unbounded repair loops from GDI.
...
- Do not run TCC reset or permission setup from GDI.
