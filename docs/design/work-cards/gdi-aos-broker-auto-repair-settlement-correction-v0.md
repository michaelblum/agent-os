# GDI Work Card: AOS Broker Auto-Repair Settlement Correction
...
- Expected local branch: create a local branch such as `gdi/aos-broker-auto-repair-settlement-v0` from `origin/main`.
...
If live readiness is blocked by TCC/input tap, stop with `human_needed` using `.docks/gdi/scripts/human-needed-tcc-reset` and report the deterministic verification already completed.
...
- live `./aos ready --json` and `./aos status --json` result, or the human-needed blocker;
