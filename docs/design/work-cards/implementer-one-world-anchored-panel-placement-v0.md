# Implementer Work Card: One-World Anchored Panel Placement V0

## Recipient

Implementer implementation / validation round.
...
- Expected output branch: `implementer/one-world-anchored-panel-placement-v0`
...
`./aos launch ... --allow-start` in this Implementer round unless Foreman explicitly
approves it later. If repo-mode readiness or input permissions block live
checks, stop with `manual_intervention` using:
...
Implementer may adjust names after reading existing conventions, but preserve this
contract:
...
Do not perform live service restart/repair in this Implementer round.
