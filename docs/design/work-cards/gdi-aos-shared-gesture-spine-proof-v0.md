# GDI Work Card: AOS Shared Gesture Spine Proof V0
...
GDI implementation and validation round.

## Branch / Base

- `branch_from`: local `main`
- `required_start_ref`: local `main` with this work card present
- `expected_output_branch`: `gdi/aos-shared-gesture-spine-proof-v0`
...
Then stop with `human_needed` and return the blocker to Foreman. Do not repair
...
If live readiness is blocked, use the GDI TCC human-needed helper and stop with
`human_needed`. Do not continue into live-dependent proof.
