# Implementer Sigil Warm Surface Lifecycle Performance V0
...
- Related contracts:
  ...
  - `docs/design/work-cards/implementer-active-sigil-canvas-clean-ownership-correction-v0.md`

## Branch / Base

- branch_from: `origin/feat/command-surface-extraction`
- required_start_ref: `origin/feat/command-surface-extraction`
- output_branch: `implementer/sigil-warm-surface-lifecycle-performance-v0`

## Fresh Context Contract

Implementer starts from a fresh context window. Do not assume branch, daemon, canvases,
...
- stale branch-scoped Sigil/toolkit content roots from prior Implementer branches do not
...
- `./aos content status --json` showed current roots plus stale branch-scoped
  roots such as `sigil_implementer_*` and `toolkit_implementer_*`.
...
from old Implementer branches. Prefer a narrowly scoped cleanup/reconciliation strategy
...
- Do not make Implementer self-accept architectural review findings; bring them back to
...
- live AOS readiness result or TCC/manual-intervention blocker;
- remaining risks and whether they require Foreman review, Operator live proof,
  or another Implementer round.
