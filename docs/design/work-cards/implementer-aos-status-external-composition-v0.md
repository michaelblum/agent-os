# Implementer AOS Status External Composition V0
...
- Branch/Base:
  ...
  - required_start_ref: local `implementer/aos-target-addressed-action-ergonomics-v0` at or after `f418bb22`
...
- Stop conditions: stop with `manual_intervention` if live verification hits repo-mode TCC, Accessibility, Input Monitoring, or inactive input-tap blockers; stop and report a blocker if public `status` cannot be preserved without adding another native primitive or moving workflow policy back into Swift.
...
- Leave broad Swift `__status` in place unless every in-repo caller is confirmed cut over and removal is explicitly safe without a native build/TCC cycle. If removal looks warranted, report it to Foreman as the next slice instead of expanding this Implementer round into Swift cleanup.
...
Live `./aos status` may be `degraded` for stale resources, active-experience drift, or other non-TCC notes. That is acceptable if the command returns valid status output and the report explains the note. If readiness is blocked by TCC/input tap, use the manual-intervention stop path above instead of looping.
...
- live `./aos status` and `./aos ready` result, or exact manual-intervention blocker;
