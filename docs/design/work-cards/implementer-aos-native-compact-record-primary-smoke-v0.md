# Implementer Work Card: AOS-Native Compact Record-Primary Smoke v0
...
- required_start_ref: `implementer/post-refactor-real-input-dogfooding-corrections-v0`
- branch/output expectation: keep work on this branch unless Implementer's relay contract requires a temporary output branch; do not reset to `origin/main`
...
Then stop with `manual_intervention`; after the human returns with `finished`, run `./aos ready --post-permission`.
