...
- Transfer kind: Implementer round, intentionally long-running ``
...
- Output expectation: work autonomously on this branch for as long as needed; commit coherent checkpoints; push only if the active Implementer/provider contract permits or the human explicitly asks.

## Foreman Override

This card intentionally overrides the usual small-slice Foreman/Implementer workflow for
this branch. Michael wants one large `` that can run for days or weeks.
...
1. Implementer runs `./aos dev build`.
...
3. Codex keeps spending tokens on readiness classification, helper ritual,
   repeated status commands, and verbose `manual_intervention` reporting.
...
```text
build passed; pause the current goal now with pause; AOS/hook should run
...
`docs/design/work-cards/implementer-input-tap-permission-reset-safety-v0.md`.
...
- live-dependent work stops with the dock-owned manual-intervention path when TCC/input
...
- if a Codex tool call runs `./aos dev build` and it succeeds, immediately
  request `pause` for the active Implementer session through the control helper;
...
and stop with a clear `manual_intervention` report.
...
- Rebuild-triggered repo-mode TCC/input-tap degradation has a deterministic
  Codex hook-level `pause` path, verified by tests.
...
- classification: pass/fail/manual_intervention;
...
- next command Implementer will run after the human returns.
