# Recent UI Live Regression Implementer Repairs V0
...
./aos show create --id integration-hub-implementer-smoke --at 120,100,980,680 --interactive --focus --url aos://toolkit/components/integration-hub/index.html
./aos show wait --id integration-hub-implementer-smoke --manifest integration-hub --timeout 5s
```

Also run the new Decision Gate safe launch path.

Capture any live evidence under `/tmp/aos-recent-ui-implementer-repairs-v0/`.
...
- whether any unresolved `NO_DAEMON`, `IPC failure`, or input-tap blocker
  remains;
- whether the next step is Operator rerun, a daemon IPC Implementer slice, or another
  toolkit fix slice.
