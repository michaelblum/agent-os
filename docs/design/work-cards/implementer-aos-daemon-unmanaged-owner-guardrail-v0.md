# Implementer Work Card: AOS Daemon Unmanaged Owner Guardrail V0
...
- `expected_output_branch`:
  `implementer/aos-daemon-unmanaged-owner-guardrail-v0`
...
Foreman correction after a Implementer stall: do not classify a process as unmanaged
...
- If the repo socket owner is not launchd-managed, or the service status does
  ...
  and report `manual_intervention` with the PID, PPID, command line, socket owner, and
