# Work Card: GDI Experience Runtime Hygiene Correction V0
...
- Source artifact: Foreman review of
  ...
  `gdi/gdi-sigil-launch-product-boundary-correction-v0`, plus Michael's live
  report that mouse events felt abnormal and an older surface flashed on screen.
- Branch/output expectation: continue from
  `origin/gdi/gdi-sigil-launch-product-boundary-correction-v0`, commit and push
  the corrected GDI branch. Do not open or merge a PR; Foreman will review.
- Stop conditions: complete, failed, human_needed, or blocker only if live
  runtime semantics require a product choice that is not in this card.

## Branch / Base

- branch_from: `origin/gdi/gdi-sigil-launch-product-boundary-correction-v0`
- required_start_ref: `origin/gdi/gdi-sigil-launch-product-boundary-correction-v0`
...
Foreman reproduced runtime hygiene problems after GDI's completion report:
...
4. `./aos experience deactivate --json` reported a vanilla AOS status item:
   ...
   status_item.toggle_url=aos://sigil_gdi_gdi_sigil_launch_product_boundary_correction_v0/renderer/index.html?toolkit-root=toolkit_gdi_gdi_sigil_launch_product_boundary_correction_v0
...
git rev-parse HEAD origin/gdi/gdi-sigil-launch-product-boundary-correction-v0
./aos status --json
