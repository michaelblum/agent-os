# Implementer Work Card: AOS Target-Addressed Action Ergonomics V0
...
- `expected_output_branch`: `implementer/aos-target-addressed-action-ergonomics-v0`
...
`implementer/toolkit-panel-live-drag-correction-v1` in:
...
Then stop with `manual_intervention`. Do not retry live coordinate checks or run raw
...
- Do not continue the old live panel drag coordinate debugging as this round's
  primary objective.
- Do not commit the interrupted `implementer/toolkit-panel-live-drag-correction-v1`
...
After completion, clear any reused Implementer goal state with clear the stale prompt state before
