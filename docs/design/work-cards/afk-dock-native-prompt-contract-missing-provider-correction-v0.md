# AFK Dock Native Prompt Contract Missing Provider Correction v0
...
Prevent missing native prompt provider declarations from crashing AFK prompt
...
native prompt message contract. The correction is about error containment and
receipt shape when the selected provider is supported by the prototype but not
declared by the target dock native subagent prompt contract.

## Branch / Base

- branch_from: `implementer/afk-dock-native subagent prompt contract-prompt-source-v0`
- required_start_ref:
  `implementer/afk-dock-native subagent prompt contract-prompt-source-v0` at
  `ceaf38ef267c20b1f8f3ffd6d4db39a8d7ca64ad`
- expected_output_branch:
  `implementer/afk-dock-native subagent prompt contract-missing-provider-correction-v0`
...
`repoRoot` is present. Current dock native prompt contracts only declare `codex`.

That means a supported non-Codex provider such as `claude` with the `operator`
dock throws:

```text
provider claude is not declared by the operator native prompt contract
```
...
- Codex/Implementer prompt construction uses the implementer native prompt contract.
- Codex/Implementer receipts preserve `provider_prompt_mode=codex_goal` and
  `provider_prompt_prefix=""`.
- Operator/Codex remains plain with empty prefix.
- Implementer contract-declared self-acceptance errors still block before typed input.
- Contract warnings remain visible and non-blocking.

Fix the missing-provider path:

- A missing provider declaration in a dock native subagent prompt contract must not escape as
...
- Existing guard receipts should retain their normal JSON shape and lifecycle
  classification.
- Do not add full Claude/Gemini native prompt provider contracts in this slice
...
- A launch/session path that uses `--provider claude --dock operator` still
  ...
  contract exception.
- Codex/Implementer contract-backed prompt tests still pass.
- Contract-declared Implementer self-acceptance errors still block before bridge typing.
