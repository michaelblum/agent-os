# Implementer AOS Ready External Composition V0
...
- Branch/Base:
  ...
  - required_start_ref: local `implementer/aos-target-addressed-action-ergonomics-v0` at or after `b94f486f`
...
- Stop conditions: stop with `manual_intervention` if live verification hits repo-mode TCC/Input Monitoring/input-tap blockers; stop and report a blocker if public `ready` cannot be preserved without adding another native primitive or moving workflow policy back into Swift.
...
Then stop with `manual_intervention` and return the blocker to Foreman. Do not run permission reset/setup, TCC reset, service restart loops, or raw launchd recovery.
...
If readiness is blocked by TCC/input tap, use the manual-intervention stop path above instead of looping.
...
- live `./aos ready` result or exact manual-intervention blocker;
