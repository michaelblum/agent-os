...
Refine the AOS native prompt message contract so the contract owns provider and
dock mechanics while Foreman routing policy owns the judgment of when Implementer is the
right tool.
...
- branch_from: current local `implementer/aos-dock-inbound-message-contract-v0` routing head
- required_start_ref: current local `implementer/aos-dock-inbound-message-contract-v0` routing head
- accepted_source_head: `64764f19d38e3f4a5e1c5c70acc186dd4bb554e2`
- expected_output_branch: `implementer/aos-dock-inbound-message-contract-policy-split-v0`
...
- Implementer clipboard payloads stay plain and preview as `<payload>`.
- Accidental Implementer  clipboard prefixes are cleaned as compatibility.
- Operator payloads stay plain with no `` prefix.
...
- Do not make every Implementer `reply exactly` or `proof only` prompt a hard contract
  error solely because it is trivial.
- Treat loop-prone one-shot Implementer prompt shapes as visible warnings or policy
  diagnostics unless they violate a real dock/protocol boundary.
- Keep true boundary violations as hard errors, for example Implementer being asked to
...
- Foreman policy decides when Implementer is the right tool.
- Foreman should use Implementer when `` adds value through autonomous iteration,
  verification, or durable work-card execution.
- Foreman should avoid Implementer for ordinary one-shot coordination unless the slice
...
- Valid Implementer transfer packet and work-card pointers still format as plain
  clipboard payloads with  provider previews.
- A Implementer one-shot `Reply with exactly...` prompt is no longer blanket-rejected
...
- A truly invalid Implementer boundary violation is still rejected.
