# Implementer Correction Round: AOS Broker JS Composition
...
- Workflow profile: `local single-checkout workflow`
- Work surface: existing PR branch `implementer/aos-target-addressed-action-ergonomics-v0`
- `branch_from`: `origin/implementer/aos-target-addressed-action-ergonomics-v0`
- `required_start_ref`: `origin/implementer/aos-target-addressed-action-ergonomics-v0`
- PR base: `implementer/radial-compact-snapshot-extraction-integration-v0` (#405 head)
...
- Leave the correction as one or more scoped local commits on
  `implementer/aos-target-addressed-action-ergonomics-v0`, or stop and report if the
...
Then report `manual_intervention` with the script output. Do not reset permissions,
...
If live readiness is blocked by TCC/input tap state, use the manual-intervention stop
