...
validation because GDI had to inspect this file before seeing the no-command
boundary. That inspection required a shell command in the tested session and
created a contract exception.

For future no-command warm-dock validation, use the inline GDI payload declared
in `.docks/gdi/inbound-contract.json` instead of sending this file pointer as
the `/goal` prompt:
