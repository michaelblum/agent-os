# GDI Sigil Status Item URL Equivalence Correction V0
...
- Single next goal: fix the review blocker in
  `gdi/sigil-status-item-stale-root-recovery-v0` where valid URL-backed Sigil
...
- Branch/output expectation: continue from
  `origin/gdi/sigil-status-item-stale-root-recovery-v0`, add a focused
  correction commit, and push the same GDI branch. Foreman will review and fold
  into PR #378 if accepted.
- Stop conditions: complete, failed, human_needed, or blocker only if the
  correction requires a product decision outside this card.

## Branch / Base

- branch_from: `origin/gdi/sigil-status-item-stale-root-recovery-v0`
- required_start_ref: `origin/gdi/sigil-status-item-stale-root-recovery-v0`
- expected output branch: `gdi/sigil-status-item-stale-root-recovery-v0`
...
=> aos://sigil_gdi_sigil_status_item_stale_root_recovery_v0/renderer/index.html?toolkit-root=toolkit_gdi_sigil_status_item_stale_root_recovery_v0

avatar-main.url
=> http://127.0.0.1:<port>/sigil_gdi_sigil_status_item_stale_root_recovery_v0/renderer/index.html?toolkit-root=toolkit_gdi_sigil_status_item_stale_root_recovery_v0
```
...
git rev-parse HEAD origin/gdi/sigil-status-item-stale-root-recovery-v0 origin/feat/command-surface-extraction
