# GDI Test Harness Ladder Prep Protocol V0
...
- Stop conditions: complete, failed, human_needed, or blocker only if current
...
- expected output branch: `gdi/test-harness-ladder-prep-protocol-v0`

This branch is separate from
`gdi/sigil-status-item-stale-root-recovery-v0`. Do not stack on the active
...
A review of `gdi/sigil-status-item-stale-root-recovery-v0` found a process
...
- keep GDI focused on implementation by giving Foreman a reusable harness-prep
...
1. Which foundational harness/workspace should a Foreman/GDI/Operator use for a
...
./aos dev recommend --json --paths tests/README.md,docs/recipes/agent-entry-paths-and-verification.md,docs/dev/workflow-rules.json
```
...
routine before routing complex GDI work.
...
- `docs/recipes/agent-entry-paths-and-verification.md` should point to the new
...
- gaps that would improve GDI consistency without slowing every task.
...
Update whichever durable guidance is most appropriate so future GDI reports can
...
Do not make every tiny GDI report verbose. Phrase this as required for
...
- Do not create a heavy mandatory design phase for every GDI task.
