# GDI AOS Doctor External Composition V0
...
- Branch/Base:
  ...
  - required_start_ref: local `gdi/aos-target-addressed-action-ergonomics-v0` at or after `17e93fcb`
...
- Stop conditions: stop with `human_needed` if live verification hits repo-mode TCC, Accessibility, Input Monitoring, or inactive input-tap blockers; stop and report a blocker if public `doctor` cannot be preserved without adding a new Swift primitive or moving public diagnostic policy back into Swift.
...
- Leave broad Swift `__doctor` in place unless every in-repo caller is confirmed cut over and removal is explicitly safe without a native build/TCC cycle. If removal looks warranted, report it to Foreman as a later native cleanup slice instead of expanding this GDI round.
...
- live `./aos doctor` and `./aos ready` result, or exact human-needed blocker;
