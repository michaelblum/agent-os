...
- expected output branch: `gdi/perceive-semantic-target-canonical-cutover-v0`
...
permissions block it, run `.docks/gdi/scripts/human-needed-tcc-reset`, stop with
`human_needed`, and resume with `./aos ready --post-permission`. The producer
