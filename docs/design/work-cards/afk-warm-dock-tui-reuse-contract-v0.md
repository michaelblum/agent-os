...
- Branch/ref gates passed on `implementer/afk-warm-dock-tui-reuse-contract-v0` at
...
- Behavior accepted: warm TUI reuse is represented as
  ...
  Codex/Implementer  dispatch, Operator plain dispatch, metadata-backed session
...
- Follow-up routed:
  ...
  for a supervised proof against the real long-lived Implementer and Operator terminals.
...
- Branch/Base:
  - `branch_from: implementer/afk-session-trigger-result-route-override-normalization-v0`
  - `required_start_ref: implementer/afk-session-trigger-result-route-override-normalization-v0`
...
- Branch/output expectation: create or reuse
  `implementer/afk-warm-dock-tui-reuse-contract-v0` from the required start ref. Commit
  and push that Implementer branch when verification passes under the active
...
- Implementer terminal with cwd `the implementer native subagent`;
- Operator terminal with cwd `the operator native subagent`.

The user normally does not restart Codex between Implementer/Operator rounds. They enter
`/clear` in the existing Codex CLI to reset the context window, then:

- for Implementer, type ``, type one space, paste Foreman's clipboard payload, and
  press Enter;
...
- The receipt must distinguish provider process identity from provider
  ...
  - mismatch if the post-reset Codex metadata resolves to the previous session
    id.
- Keep Implementer and Operator dispatch shapes distinct:
  - Codex/Implementer dispatch starts with  and then the Foreman payload;
  - Operator dispatch is the plain Foreman payload, with no `` prefix;
  - the first character typed after reset should be `/` for Implementer and the first
    payload character for Operator.
- Keep `/clear` and clear the stale prompt state separate:
  - `/clear` is the normal context reset command;
  - clear the stale prompt state is only a fallback/cleanup command for stale completed goal
...
git rev-parse HEAD implementer/afk-session-trigger-result-route-override-normalization-v0 8265ffc8dfe48f58b1f761657a01c1c9de030ed4
./aos ready
...
- Do not drive the user's real Foreman, Implementer, or Operator terminals.
...
- Do not collapse `/clear` and clear the stale prompt state into one command.
...
- receipt examples for Codex/Implementer and Operator dispatch shapes;
...
- explicit statement that no real dock terminal was driven, no live provider
  ...
  beyond the expected Implementer branch push.
