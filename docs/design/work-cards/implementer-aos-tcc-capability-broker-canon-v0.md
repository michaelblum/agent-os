ss# Implementer Work Card: AOS TCC Capability Broker Canon V0
...
- Required start ref: current local checkout in `/Users/Michael/Code/agent-os`, branch `implementer/aos-target-addressed-action-ergonomics-v0`, at or after `e0992f8d`.
...
- the implementer native subagent instructions - Implementer binary/native boundary.
- `docs/adr/0013-aos-execution-model-v0.md` - current execution model ADR.
- `src/main.swift` and `src/shared/external-command-dispatch.swift` - inspect only for architectural accuracy, not for behavior changes in this slice.
...
- one shared native tooling context.
...
- the implementer native subagent instructions: Implementer must stop/report when a goal appears to require Swift but could be solved by externalizing behavior or adding a smaller stable primitive. Implementer still must not rebuild or mutate `./aos`.
...
It should lay out the next slices without implementing them:
...
Each slice should include likely files, stop conditions, and machine-checkable acceptance evidence.

## Scope

Docs and governance only. No source behavior refactor in this slice.
...
- Do not edit Swift, JS, Python, shell source, schemas, or tests in this slice except for documentation examples that are clearly docs-only.
...
rg -n "All capability ships inside the unified|public command behavior.*Swift binary|subcommand groups.*inside the binary|source of truth for public command behavior" \
  ARCHITECTURE.md docs/dev/command-surface.md docs/adr src/AGENTS.md CONTEXT-MAP.md AGENTS.md .docks/foreman/AGENTS.md the implementer native subagent instructions
```

Also run any no-build docs checks recommended by `./aos dev recommend`. If the router recommends a rebuild, do not rebuild; report that the recommendation is over-broad for this docs-only slice.
...
- any proposed next Implementer slice for inventory/extraction.
