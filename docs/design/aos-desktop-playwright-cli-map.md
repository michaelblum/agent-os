...
| Menu-item invocation needs command-truth coverage | Shipped as pid-scoped app menu `aos do menu --path ...`; AOS-owned status-item menus use `aos experience menu invoke` | native AX/runtime adapter plus experience mounted-surface dispatch | Low | Keep app-menu dry-run path and AOS-owned status-item dry-run path current; do not imply arbitrary third-party menu-extra scraping. |
...
| Maintain menu item invocation | `aos do menu --pid <pid> --path File,Save`; `aos experience menu invoke <id> --item <item-id>` for AOS-owned status-item entries | Must validate app menu path/enabled state or experience menu projection before dispatch. |
