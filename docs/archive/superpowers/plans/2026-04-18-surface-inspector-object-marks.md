...
> **Superseded for active work on 2026-04-18.** Stop here unless you were explicitly asked to continue the original thumbnail/SVG/icon/capture design. Use [docs/superpowers/plans/2026-04-18-surface-inspector-pivot.md](./2026-04-18-surface-inspector-pivot.md) as the current source of truth.

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship consumer-published ephemeral object marks in `surface-inspector` — minimap dots + indented list rows keyed per parent canvas — per the approved spec at `docs/superpowers/specs/2026-04-18-surface-inspector-object-marks-design.md`.
...
**New files:**
- `packages/toolkit/components/surface-inspector/marks/normalize.js` — `normalizeMarks`, `sanitizeSvg`, `stableColorForId`.
- `packages/toolkit/components/surface-inspector/marks/reconcile.js` — `diffAndReconcile`, `bumpEpoch`.
- `packages/toolkit/components/surface-inspector/marks/scheduler.js` — `createScheduler` (tick lifecycle + TTL sweep + Tier 3 dispatch).
- `packages/toolkit/components/surface-inspector/marks/render.js` — `renderMarkOnMinimap`, `renderMarkListRow`, `renderDefaultShape`, `renderPrimitiveShape`.
- `packages/toolkit/components/surface-inspector/marks/controller.js` — `createMarksController` glue that wires message handler, state, scheduler, and render into `CanvasInspector`.
- `tests/toolkit/surface-inspector-marks-normalize.test.mjs`
- `tests/toolkit/surface-inspector-marks-reconcile.test.mjs`
- `tests/toolkit/surface-inspector-marks-scheduler.test.mjs`
- `tests/toolkit/surface-inspector-marks-render.test.mjs`
- `tests/surface-inspector-object-marks.sh` — integration shell test (emit + assert state + TTL + clear).
- `tests/toolkit/surface-inspector-marks-tier3.test.mjs` — Part II only.
- `tests/surface-inspector-object-marks-tier3.sh` — Part II only.
...
- `packages/toolkit/components/surface-inspector/index.js` — subscribe to `canvas_object.marks`; route messages to `MarksController`; call its render hooks inside the existing `rerender()`.
- `packages/toolkit/components/surface-inspector/styles.css` — indented mark list rows + mark swatch.
...
- `packages/toolkit/components/surface-inspector/index.js` — pattern reference for `onMessage` + `rerender`.
...
**Files:**
- Create: `packages/toolkit/components/surface-inspector/marks/normalize.js`
- Test: `tests/toolkit/surface-inspector-marks-normalize.test.mjs`
...
import {
  normalizeMarks,
  stableColorForId,
} from '../../packages/toolkit/components/surface-inspector/marks/normalize.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-normalize.test.mjs`
  ...
  for (const raw of objects) {
    ...
    if (!id) {
      if (!sawMissingId) {
        ...
        if (!warnedMissingIdByCanvas.has(key)) {
          ...
          warn(`[surface-inspector] mark dropped: missing id in canvas ${canvasId}`);
        }
      }
      continue;
    }
    if (seen.has(id)) {
      ...
      if (!warnedDupByKey.has(key)) {
        ...
        warn(`[surface-inspector] duplicate mark id dropped: ${canvasId}:${id}`);
      }
      continue;
    }
    seen.add(id);
...
```bash
git add packages/toolkit/components/surface-inspector/marks/normalize.js tests/toolkit/surface-inspector-marks-normalize.test.mjs
git commit -m "feat(surface-inspector): normalizeMarks with required id, uniqueness, stable color"
```
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/marks/normalize.js` — add `sanitizeSvg`.
- Modify: `tests/toolkit/surface-inspector-marks-normalize.test.mjs` — add sanitizer tests.
...
```js
import { sanitizeSvg } from '../../packages/toolkit/components/surface-inspector/marks/normalize.js';
...
Append to `packages/toolkit/components/surface-inspector/marks/normalize.js`:
...
```bash
git add packages/toolkit/components/surface-inspector/marks/normalize.js tests/toolkit/surface-inspector-marks-normalize.test.mjs
git commit -m "feat(surface-inspector): SVG sanitizer for mark shape payloads"
```
...
**Files:**
- Create: `packages/toolkit/components/surface-inspector/marks/reconcile.js`
- Test: `tests/toolkit/surface-inspector-marks-reconcile.test.mjs`
...
import {
  createReconciler,
  iconSignature,
} from '../../packages/toolkit/components/surface-inspector/marks/reconcile.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-reconcile.test.mjs`
Expected: FAIL — module does not exist.

- [ ] **Step 3: Implement reconciler (Tier 1/2 paths + stub Tier 3 for later)**

```js
// packages/toolkit/components/surface-inspector/marks/reconcile.js
...
```bash
git add packages/toolkit/components/surface-inspector/marks/reconcile.js tests/toolkit/surface-inspector-marks-reconcile.test.mjs
git commit -m "feat(surface-inspector): diffAndReconcile with epoch-guarded Tier 3 seeding"
```
...
**Files:**
- Modify: `tests/toolkit/surface-inspector-marks-reconcile.test.mjs`
...
```bash
git add tests/toolkit/surface-inspector-marks-reconcile.test.mjs
git commit -m "test(surface-inspector): Tier 3 transition rules for diffAndReconcile"
```
...
**Files:**
- Create: `packages/toolkit/components/surface-inspector/marks/scheduler.js`
- Test: `tests/toolkit/surface-inspector-marks-scheduler.test.mjs`
...
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { createScheduler } from '../../packages/toolkit/components/surface-inspector/marks/scheduler.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-scheduler.test.mjs`
Expected: FAIL — module missing.

- [ ] **Step 3: Implement scheduler**

```js
// packages/toolkit/components/surface-inspector/marks/scheduler.js
...
```bash
git add packages/toolkit/components/surface-inspector/marks/scheduler.js tests/toolkit/surface-inspector-marks-scheduler.test.mjs
git commit -m "feat(surface-inspector): scheduler tick with TTL sweep + tick lifecycle"
```
...
**Files:**
- Create: `packages/toolkit/components/surface-inspector/marks/render.js`
- Test: `tests/toolkit/surface-inspector-marks-render.test.mjs`
...
import {
  renderMarkVisual,
  DEFAULT_SHAPE_SVG,
} from '../../packages/toolkit/components/surface-inspector/marks/render.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-render.test.mjs`
Expected: FAIL — module missing.
...
```bash
git add packages/toolkit/components/surface-inspector/marks/render.js tests/toolkit/surface-inspector-marks-render.test.mjs
git commit -m "feat(surface-inspector): mark visual renderer (default, primitives, SVG, icon)"
```
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/marks/render.js` — add `renderMinimapMarks`.
- Modify: `tests/toolkit/surface-inspector-marks-render.test.mjs` — add layout tests.

- [ ] **Step 1: Write the failing tests**

Append to `tests/toolkit/surface-inspector-marks-render.test.mjs`:

```js
import { renderMinimapMarks } from '../../packages/toolkit/components/surface-inspector/marks/render.js';
...
```bash
git add packages/toolkit/components/surface-inspector/marks/render.js tests/toolkit/surface-inspector-marks-render.test.mjs
git commit -m "feat(surface-inspector): minimap mark placement helper"
```
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/marks/render.js` — add `renderMarkListRows`.
- Modify: `tests/toolkit/surface-inspector-marks-render.test.mjs` — add list-row tests.
...
```js
import { renderMarkListRows } from '../../packages/toolkit/components/surface-inspector/marks/render.js';
...
```bash
git add packages/toolkit/components/surface-inspector/marks/render.js tests/toolkit/surface-inspector-marks-render.test.mjs
git commit -m "feat(surface-inspector): list row renderer for indented marks"
```
...
**Files:**
- Create: `packages/toolkit/components/surface-inspector/marks/controller.js`

- [ ] **Step 1: Write the failing integration test**

Create `tests/toolkit/surface-inspector-marks-controller.test.mjs`:

```js
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { createMarksController } from '../../packages/toolkit/components/surface-inspector/marks/controller.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-controller.test.mjs`
Expected: FAIL — controller missing.

- [ ] **Step 3: Implement the controller**

```js
// packages/toolkit/components/surface-inspector/marks/controller.js
...
```bash
git add packages/toolkit/components/surface-inspector/marks/controller.js tests/toolkit/surface-inspector-marks-controller.test.mjs
git commit -m "feat(surface-inspector): marks controller binding state, reconcile, scheduler, render"
```
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/index.js`
...
- [ ] **Step 2: Edit `packages/toolkit/components/surface-inspector/index.js`**
...
Run: `node --test tests/toolkit/surface-inspector.test.mjs tests/toolkit/surface-inspector-marks-*.test.mjs`
...
git commit -m "feat(surface-inspector): wire MarksController into subscribe + render pipeline"
```
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/styles.css`
...
Append to `packages/toolkit/components/surface-inspector/styles.css`:
...
```bash
git add packages/toolkit/components/surface-inspector/styles.css
git commit -m "feat(surface-inspector): styles for indented mark rows + minimap marks"
```
...
sleep 3
./aos show eval --id surface-inspector --js 'JSON.stringify(Object.keys(window.__canvasInspectorState.marksByCanvas))'
```
Expected: output includes `"avatar-main"`.

Tear down:
```bash
./aos show remove --id avatar-main --id surface-inspector
pkill -f 'aos serve --idle-timeout' || true
```
...
**Files:**
- Create: `tests/surface-inspector-object-marks.sh`
...
echo "PASS: surface-inspector object marks (Part I)"
```

- [ ] **Step 2: Make executable and run**

```bash
chmod +x tests/surface-inspector-object-marks.sh
bash tests/surface-inspector-object-marks.sh
```
Expected: `PASS: surface-inspector object marks (Part I)`.

- [ ] **Step 3: Commit**

```bash
git add tests/surface-inspector-object-marks.sh
...
Consumers can publish ephemeral "object marks" that `surface-inspector` renders on its minimap and in an indented list row under the parent canvas.
...
**Files:**
- Modify: `packages/toolkit/components/surface-inspector/marks/controller.js`
- Create: `packages/toolkit/components/surface-inspector/marks/capture-client.js`
- Test: `tests/toolkit/surface-inspector-marks-tier3.test.mjs`

- [ ] **Step 1: Write the failing test**

```js
// tests/toolkit/surface-inspector-marks-tier3.test.mjs
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { createMarksController } from '../../packages/toolkit/components/surface-inspector/marks/controller.js';
...
Run: `node --test tests/toolkit/surface-inspector-marks-tier3.test.mjs`
Expected: FAIL — `commitCapture` not exposed.

- [ ] **Step 3: Implement `commitCapture` in controller**

Edit `packages/toolkit/components/surface-inspector/marks/controller.js`. Inside `createMarksController`, after `onCanvasRemoved`, add:
...
```bash
git add packages/toolkit/components/surface-inspector/marks/controller.js tests/toolkit/surface-inspector-marks-tier3.test.mjs
git commit -m "feat(surface-inspector): Tier 3 commitCapture with stale-response guard"
```
...
```js
// packages/toolkit/components/surface-inspector/marks/capture-client.js
...
Edit `packages/toolkit/components/surface-inspector/index.js`:
...
```bash
git add packages/toolkit/components/surface-inspector/marks/capture-client.js packages/toolkit/components/surface-inspector/index.js
git commit -m "feat(surface-inspector): capture client wiring Tier 3 over daemon see.capture"
```
...
**Files:**
- Create: `tests/surface-inspector-object-marks-tier3.sh`
...
echo "PASS: surface-inspector object marks (Tier 3)"
```

- [ ] **Step 2: Run it**

```bash
chmod +x tests/surface-inspector-object-marks-tier3.sh
bash tests/surface-inspector-object-marks-tier3.sh
```
...
```bash
git add tests/surface-inspector-object-marks-tier3.sh
...
**Files:**
- Modify: `tests/toolkit/surface-inspector-marks-tier3.test.mjs`
...
```bash
git add tests/toolkit/surface-inspector-marks-tier3.test.mjs
git commit -m "test(surface-inspector): one-in-flight invariant under simulated long capture"
```
...
```bash
node --test tests/toolkit/surface-inspector-marks-*.test.mjs tests/toolkit/surface-inspector.test.mjs
bash tests/surface-inspector-object-marks.sh
bash tests/surface-inspector-object-marks-tier3.sh    # may SKIP without screen recording
```
