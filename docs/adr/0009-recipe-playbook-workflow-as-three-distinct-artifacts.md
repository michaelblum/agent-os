...
ADR 0013 defines the formal AOS Execution Model V0 and narrows **Recipe** to
source-backed executable procedures. ADR 0040 and active Work Record V1 own the
neutral evidence boundary. Markdown guidance lives under `docs/guides/` as
Guides/SOPs, not Recipes.
...
Composition: Workflows may invoke Recipes, caller-selected harnesses, agent
tasks, gates, or human decisions; Recipes should not invoke Workflows (anything
orchestrating multi-system gates and child runs *is* a Workflow). Active Work
Record V1 uses `origin: { kind: "ad_hoc" | "recipe" | "workflow", ref:
<handle> | null }` for executable origins. Markdown Guides/SOPs and Playbooks
that guided a run without executing are cited via `references[]` with
`relationship: "guided_by"`, not as origin. Work Record V0 is frozen history.
