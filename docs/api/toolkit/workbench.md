...
`surface_inspector`, `radial_menu`, and `unknown`. Unrecognized input normalizes
to `unknown`; current records and serialized output emit only this vocabulary.
...
The fixture and CLI prove a neutral fixture-backed work-card artifact path:
...
state-scoped `data-aos-ref`, local DOM identity in `provenance.dom_id` from
...
domain-specific fields or treating annotations as fixed UI positions.
...
locator repair, and report/export/workflow execution remain outside this tree
contract unless a separate bounded adapter explicitly owns that work.
...
The default Markdown spatial tree seed is generated by:

```bash
node scripts/markdown-spatial-subject-tree.mjs --input tests/fixtures/html-workbench-expression/sample-work-card.md
```

The Markdown spatial tree CLI requires an explicit input. The default output is
`docs/design/fixtures/spatial-subject-tree-v0/markdown-spatial-subject-tree.json`.
...
Markdown/report artifact. The fixture uses one stable
`aos.workbench.subject` with `subject_type:
...
artifact's own `files[]`, including source paths, Browser Evidence Capture V0
planning manifest skeletons, capture manifests, registries, local fixture page
paths, and crop asset paths when present. For Browser Evidence fixtures, source
metadata can also carry a read-only planning-vs-captured coverage summary. The
helper
...
request IDs, and per-company/source-category coverage rows.
...
projection reads a root page's stage-contract table and resolves linked wiki
pages into `wiki:<path>` child subjects. This is a view/model contract only:
invocation stays with existing `aos wiki invoke` or agent-driven workflow
