...
4. If it has landed, resume according to the active dev workflow profile in
   `docs/dev/active-profile.json` and `docs/dev/workflow-profiles.json`. Do not
   assume a profile from this primer; use the active profile unless the user
   gives a safe session-specific override.
