...
4. **Visual-only global decoration or diagnostic layer.** Use the shared
   ...
   include transfer outlines, drag ghosts, telemetry overlays, and diagnostic
   marks that do not consume input.
...
| Drag transfer visuals | `packages/toolkit/panel/drag-transfer.js` publishes passive transfer outlines to the shared DesktopWorld stage. | Visual-only global decoration or diagnostic layer. | Acceptable | #261 | Keep as the precedent for passive stage visuals; add input only if the transfer affordance becomes actionable. |
