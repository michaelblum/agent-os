...
A browser tab is a first-class target for `see`, `do`, and `show` verbs. The adapter lives entirely in the CLI process (`src/browser/`) and shells out to Microsoft's `playwright-cli`; the daemon is unchanged. Direct browser targets use the grammar `browser:<session>[/<ref>]` where `<session>` is the `playwright-cli -s=<name>` session (registered as an aos focus channel) and `<ref>` is a volatile adapter ref from current browser perception. Public target semantics distinguish an ephemeral Observation Ref `(state_id, ref)` from a Locator that re-resolves at action time. The current browser implementation predates that split: agents can capture `aos see capture browser:<session> --save --mode som --workspace <id>`, inspect `aos see refs`, and act through snapshot-qualified saved handles such as `ref:<snapshot-id>:<ref>`; current saved-handle dispatch validates the browser target before routing through the underlying direct `browser:<session>/<ref>` action. Overlays anchored to browser elements still take direct Target-with-Ref input and are static in v1 — they follow Chrome window movement (via `anchor_window`) but not page scroll; agents re-issue `aos show update --anchor-browser …` to re-anchor. Historical adapter design context is archived at `docs/archive/superpowers/specs/2026-04-24-playwright-browser-adapter-design.md`.
