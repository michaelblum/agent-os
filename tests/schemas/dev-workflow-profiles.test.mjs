...
test('canonical profiles preserve the expected built-in examples', async () => {
  ...
  assert.ok(profiles.has('remote_branch_relay'));
  assert.ok(profiles.has('local_checkpoint'));
  assert.ok(profiles.has('hybrid_trunk'));
  assert.ok(profiles.has('github_flow'));
  assert.ok(profiles.has('gitflow'));
  assert.equal(profiles.get('remote_branch_relay')?.direct_main_default, false);
  assert.equal(profiles.get('remote_branch_relay')?.pull_request_required, false);
  assert.equal(profiles.get('remote_branch_relay')?.default_work_surface, 'short-lived topic branch from main');
  assert.equal(profiles.get('remote_branch_relay')?.commit_strategy, 'scoped commits on a topic/<slug> branch; push branch to origin at completion when explicitly authorized');
  assert.equal(profiles.get('remote_branch_relay')?.branch_naming_convention, 'topic/<slug>');
  assert.equal(profiles.get('remote_branch_relay')?.push_at_completion, true);
  assert.equal(profiles.get('local_checkpoint')?.direct_main_default, false);
  assert.equal(profiles.get('local_checkpoint')?.pull_request_required, false);
  assert.equal(profiles.get('local_checkpoint')?.push_at_completion, false);
  assert.ok(profiles.get('local_checkpoint')?.branch_strategy.includes('never create linked git worktrees'));
  ...
  assert.ok(manifest.axes?.session_identity?.includes('session'));
  assert.ok(manifest.axes?.execution_surface?.includes('work happens'));
  ...
  assert.ok(manifest.axes?.handoff_boundary?.includes('boundary'));
});
