...
test('canonical profiles preserve the expected built-in examples', async () => {
  ...
  assert.ok(profiles.has('local_single_checkout'));
  ...
  assert.equal(profiles.get('agentic_relay')?.commit_strategy, 'scoped commits on a implementer/<slug> branch; push branch to origin at completion');
  assert.equal(profiles.get('agentic_relay')?.branch_naming_convention, 'implementer/<work-card-slug>');
  assert.equal(profiles.get('agentic_relay')?.implementer_push_authority, true);
  assert.equal(profiles.get('local_single_checkout')?.direct_main_default, false);
  assert.equal(profiles.get('local_single_checkout')?.pull_request_required, false);
  assert.equal(profiles.get('local_single_checkout')?.implementer_push_authority, false);
  assert.ok(profiles.get('local_single_checkout')?.branch_strategy.includes('never create linked git worktrees'));
  ...
  assert.ok(manifest.axes?.entry_path?.includes('tooling context'));
