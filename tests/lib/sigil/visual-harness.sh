...
SIGIL_VISUAL_HARNESS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SIGIL_VISUAL_HARNESS_ROOT="$(cd "$SIGIL_VISUAL_HARNESS_DIR/../../.." && pwd)"

source "$SIGIL_VISUAL_HARNESS_DIR/../visual-harness.sh"
...
aos_visual_assert_sigil_renderer_fresh() {
  ...
  if [[ -z "$commit_time" ]]; then
    commit_time="$(git -C "$SIGIL_VISUAL_HARNESS_ROOT" show -s --format=%cI HEAD)"
  fi
...
aos_visual_seed_sigil() {
  ...
  local aos_bin
  aos_bin="$(aos_visual_aos)"
  AOS_BIN="$aos_bin" AOS_RUNTIME_MODE="$mode" "$SIGIL_VISUAL_HARNESS_ROOT/apps/sigil/sigilctl-seed.sh" >/dev/null
}
...
aos_visual_prepare_live_roots() {
  ...
  aos_ensure_content_roots_live "$aos_bin" \
    "$toolkit_key" "$SIGIL_VISUAL_HARNESS_ROOT/packages/toolkit" \
    "$sigil_key" "$SIGIL_VISUAL_HARNESS_ROOT/apps/sigil"
  aos_visual_assert_live_content_root "$toolkit_key" "$SIGIL_VISUAL_HARNESS_ROOT/packages/toolkit"
  aos_visual_assert_live_content_root "$sigil_key" "$SIGIL_VISUAL_HARNESS_ROOT/apps/sigil"
}
...
aos_visual_configure_sigil_status_item() {
  ...
  sigil_configure_status_item "$aos_bin" "$SIGIL_VISUAL_HARNESS_ROOT/apps/sigil" "$SIGIL_VISUAL_HARNESS_ROOT/packages/toolkit" "$avatar_id"
}
