...
before = json.loads(sys.argv[1]).get("sessions", [])
after = json.loads(sys.argv[2]).get("sessions", [])
...
if any("session-stop.sh" in command for command in stop_commands):
    raise SystemExit(f"FAIL: codex Stop hook must not unregister session presence: {stop_commands}")
PY
