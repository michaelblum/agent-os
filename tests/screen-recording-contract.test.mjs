...
test('durability and custody ordering is producer-backed and retain is specifically unavailable', async () => {
  ...
  assert.match(state, /enum AOSOperationPublicProgressProjection/u)
  assert.match(unified, /AOSOperationPublicProgressProjection\.progress\(operation\)/u)
  assert.match(unified, /AOSOperationPublicProgressProjection\.terminal\(/u)
