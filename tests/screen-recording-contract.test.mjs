...
test('durability and custody ordering is producer-backed and retain is specifically unavailable', async () => {
  ...
  const lifecyclePrepare = start.indexOf('claimAdmission = try prepare(')
  const pendingOwner = start.indexOf('runtimes[claimAdmission.publicAdmission.operation] = pendingRuntime')
  const lifecycleUnlock = start.indexOf('lock.unlock()', pendingOwner)
  ...
  assert.ok(lifecyclePrepare < pendingOwner && pendingOwner < lifecycleUnlock)
  ...
  assert.match(preparation, /preparedPublicationObserver\(operation\.identity\)[\s\S]*registry\.prepareStream/u)
  assert.match(preparation, /terminalizeScreenRecordingPreparationFailure/u)
  ...
  assert.match(registry, /func terminalizeScreenRecordingPreparationFailure\(/u)
