...
PREFIX="aos-content-command-preflight"
...
run_against_limited_mock "status" ./aos content status --json
run_against_limited_mock "wait" ./aos content wait --timeout 0.2s --json
...
expected = [
    ("system.preflight aos content status", "content.status"),
    ("system.preflight aos content wait", "content.status"),
]
cursor = 0
...
echo "PASS: content commands preflight runtime.daemon before IPC"
echo "PASS"
