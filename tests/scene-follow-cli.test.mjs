...
test('scene-follow initializes and verifies the stage before admitting subscriptions', async () => {
  const source = await readFile(new URL('../src/daemon/desktop-world-scene-transport-controller.swift', import.meta.url), 'utf8')
  const start = source.indexOf('func follow(')
  const end = source.indexOf('\n    private func response(', start)
  assert.notEqual(start, -1, 'follow is missing')
  assert.notEqual(end, -1, 'follow response boundary is missing')
  const body = source.slice(start, end)
  const validate = body.indexOf('Invalid scene event subscription')
  const unsubscribe = body.indexOf('if op == "unsubscribe"')
  ...
  assert.ok(unsubscribe < ensure, 'unsubscription must not create a stage during teardown')
