...
import StepDescriptorWorkbench from '../../packages/toolkit/components/step-descriptor-workbench/index.js';
import {
  ...
  stepDescriptorWorkbenchForbiddenControls,
...
} from '../../packages/toolkit/components/step-descriptor-workbench/semantics.js';
import {
...
function workflowGate() {
  return {
    ...
    token: 'workflow-gate-token:step-descriptor-workbench-test',
  };
}

function loadedState() {
  return createStepDescriptorWorkbenchState({
...
function assertNoForbiddenControls(subject) {
  const forbidden = stepDescriptorWorkbenchForbiddenControls(subject);
...
test('Step Descriptor Workbench V0 starts as a named fixture-backed report-only shell state', () => {
  const state = loadedState();
  const snapshot = stepDescriptorWorkbenchSnapshot(state);

  assert.equal(snapshot.surface, STEP_DESCRIPTOR_WORKBENCH_SURFACE);
  assert.equal(snapshot.url, STEP_DESCRIPTOR_WORKBENCH_URL);
  assert.equal(snapshot.fixture_loaded, true);
  assert.equal(snapshot.status, 'ready');
  assert.equal(snapshot.step_summary.id, 'step-descriptor:browser-click-status');
...
test('Step Descriptor Workbench V0 loads fixtures through its launch/message contract', () => {
  const state = createStepDescriptorWorkbenchState();
  const result = loadStepDescriptorWorkbenchFixture(state, {
  ...
  assert.equal(result.type, STEP_DESCRIPTOR_WORKBENCH_MESSAGE_TYPES.load);
  assert.equal(result.status, 'loaded');
  assert.equal(snapshot.fixture_loaded, true);
  assert.equal(snapshot.step_summary.workflow_ref, 'workflow:browser-live-action-status');
  assert.equal(
...
test('Step Descriptor Workbench V0 rejects simulation until an explicit gate ref and token are present', () => {
  const state = loadedState();
...
test('Step Descriptor Workbench V0 rejects undeclared workflow gate refs', () => {
  const state = loadedState();
  setStepDescriptorWorkbenchWorkflowGate(state, {
...
test('Step Descriptor Workbench V0 simulates exactly one gated saved-evidence step', () => {
  const state = loadedState();
  setStepDescriptorWorkbenchWorkflowGate(state, workflowGate());
  const result = simulateStepDescriptorWorkbench(state);
  const snapshot = stepDescriptorWorkbenchSnapshot(state);
  ...
  assert.equal(state.record.origin.kind, 'workflow');
  assert.equal(state.record.execution_map.steps.length, 1);
  assertNoForbiddenControls(snapshot.subject);
});

test('Step Descriptor Workbench V0 hands off emitted records to the existing read-only Work Record workbench model', () => {
  const state = loadedState();
  setStepDescriptorWorkbenchWorkflowGate(state, workflowGate());
  simulateStepDescriptorWorkbench(state);

  const openResult = openStepDescriptorWorkbenchWorkRecord(state);
  const snapshot = stepDescriptorWorkbenchSnapshot(state);
  ...
  assert.equal(snapshot.work_record_open.open_message.source.kind, 'browser_step_descriptor_prototype');
...
test('Step Descriptor Workbench V0 exposes stable semantic refs and no replay/repair/macro controls', async () => {
  const shell = StepDescriptorWorkbench();
  ...
  assert.equal(refs.root, stepDescriptorWorkbenchAosRef('root'));
  assert.equal(refs.gateRef, stepDescriptorWorkbenchAosRef('gate-ref'));
  assert.equal(refs.gateToken, stepDescriptorWorkbenchAosRef('gate-token'));
  assert.equal(refs.simulate, stepDescriptorWorkbenchAosRef('simulate'));
  assert.equal(refs.openWorkRecord, stepDescriptorWorkbenchAosRef('open-work-record'));
  assert.ok(shell.manifest.accepts.includes(STEP_DESCRIPTOR_WORKBENCH_MESSAGE_TYPES.load));
  assert.ok(shell.manifest.accepts.includes(STEP_DESCRIPTOR_WORKBENCH_MESSAGE_TYPES.workflowGateSet));
  assert.ok(shell.manifest.accepts.includes(STEP_DESCRIPTOR_WORKBENCH_MESSAGE_TYPES.simulateRequested));
  assert.ok(shell.manifest.accepts.includes(STEP_DESCRIPTOR_WORKBENCH_MESSAGE_TYPES.workRecordOpenRequested));
  assert.match(indexHtml, /Step Descriptor Workbench V0/);
