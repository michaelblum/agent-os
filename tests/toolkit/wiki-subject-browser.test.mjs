...
test('wiki subject browser catalog opens Employer Brand artifact bundle with report preview and evidence handoff', async () => {
  ...
  assert.equal(previewSnapshot.selected_work_record_summary.evidence_ref_count, 14);
  ...
  assert.equal(evidenceSnapshot.linked_work_record_open.workbench_snapshot.diagnostics.evidence_count, 14);
  ...
  assert.equal(evidenceSnapshot.selected_work_record_summary.verified_claim_count, 9);
});
