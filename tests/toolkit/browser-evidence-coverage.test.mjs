...
test('browser evidence coverage helper compares Employer Brand planning manifest against captured registry', async () => {
  ...
  assert.equal(summary.matched_request_count, 0);
  assert.equal(summary.missing_planned_count, 21);
  assert.equal(summary.extra_captured_count, 3);
  assert.equal(summary.missing_planned_requests.length, 21);
  assert.equal(summary.extra_captured_requests.length, 3);
  assert.equal(summary.missing_planned_request_ids[0], 'symphony_talent_careers_site_planning');
  assert.deepEqual(summary.extra_captured_request_ids, [
    'symphony_talent_fixture_evp_hero',
    'phenom_fixture_culture_card',
    'radancy_fixture_proof_block',
  ]);
  ...
  assert.deepEqual({
    ...
    missing_planned_count: 1,
    extra_captured_count: 1,
    coverage_status: 'captured_without_matching_request_id',
  });
