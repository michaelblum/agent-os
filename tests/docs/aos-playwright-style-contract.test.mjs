...
test('AOS capability docs keep saved-ref observe-act loop separate from waits', async () => {
  ...
  for (const phrase of [
    ...
    'Add `--dry-run` only when the caller wants identical resolution with mutation omitted; it is not action permission.',
    './aos see refs --workspace default --diff before..after --expect change --json',
    'Fresh perception still comes from the action loop',
