...
test('aos skills install dry-run reports planned writes and does not mutate target', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.selected, 13);
...
test('aos skills install writes the default installable pack', async () => {
  ...
  try {
    ...
    assert.equal(installed.summary.selected, 13);
    assert.equal(installed.summary.states_after.ok, 13);
    ...
    assert.equal(existsSync(path.join(target, 'aos-desktop', 'SKILL.md')), true);
    assert.equal(existsSync(path.join(target, 'aos-verification', 'SKILL.md')), true);
