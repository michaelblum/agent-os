...
test('documented register-follow, update, inspect, dry-run, and invoke use truthful multi-connection ownership', async () => {
  ...
  const server = await listenFake(stateRoot, (socket, request) => {
    ...
    if (request.action === 'register') {
      leaseSocket = socket
      socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'ok', owner: descriptor.owner, item_id: descriptor.item_id, generation: 7, descriptor_revision: 3, anchor } })}\n`)
      ...
      socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'ok', owner: descriptor.owner, item_id: descriptor.item_id, generation: 7, previous_descriptor_revision: 3, descriptor_revision: activeRevision, updated: true, anchor } })}\n`)
      socket.end()
      ...
      socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'ok', state: { owner: descriptor.owner, item_id: descriptor.item_id, generation: 7, descriptor_revision: activeRevision, anchor } } })}\n`)
      socket.end()
    } else if (request.action === 'invoke_dry_run') {
      socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'dry_run', owner: descriptor.owner, item_id: descriptor.item_id, action_id: 'summon', generation: 7, descriptor_revision: activeRevision, anchor } })}\n`)
      socket.end()
    } else if (request.action === 'invoke') {
      socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'ok', owner: descriptor.owner, item_id: descriptor.item_id, action_id: 'summon', generation: 7, descriptor_revision: activeRevision, anchor } })}\n`)
      socket.end()
    }
  })
...
test('register bounds events that arrive before the registration result', async () => {
  ...
  const server = await listenFake(stateRoot, (socket, request) => {
    ...
    socket.write(`${JSON.stringify({ ref: request.ref, data: { status: 'ok', generation: 7, descriptor_revision: 3 } })}\n`)
  })
...
test('CLI settles daemon error, malformed response, and disconnect failures', async () => {
  for (const scenario of ['error', 'malformed', 'disconnect']) {
    const stateRoot = fs.mkdtempSync(`/private/tmp/aos-status-item-${scenario}-`)
    const server = await listenFake(stateRoot, (socket, request) => {
      if (scenario === 'error') {
        socket.end(`${JSON.stringify({ ref: request.ref, status: 'error', code: 'STATUS_ITEM_TEST_ERROR', error: 'test daemon error' })}\n`)
