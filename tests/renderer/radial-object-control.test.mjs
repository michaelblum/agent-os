...
test('wiki brain object registry advertises scene, shell, fiber, and fractal tree controls', () => {
  ...
  const scene = registry.objects.find((object) => object.object_id === WIKI_BRAIN_SCENE_OBJECT_ID)
  assert.equal(scene.name, 'Wiki Brain Scene')
  assert.deepEqual(scene.transform.rotation_degrees, { x: 0, y: 0, z: 0 })
...
test('wiki brain scene transform rotates the layered object root', () => {
  ...
  const result = applyWikiBrainTransformPatch(config, {
    ...
    target: {
      ...
      object_id: WIKI_BRAIN_SCENE_OBJECT_ID,
    },
    patch: {
  ...
  assert.deepEqual(result.transform.rotation_degrees, { x: 0, y: 18, z: 0 })

  const scene = buildWikiBrainObjectRegistry(config, { canvasId: 'avatar-main' })
    .objects.find((object) => object.object_id === WIKI_BRAIN_SCENE_OBJECT_ID)
  assert.deepEqual(scene.transform.rotation_degrees, { x: 0, y: 18, z: 0 })
})
