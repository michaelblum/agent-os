...
test('Sigil main delegates dictation host messages and key input to voice runtime', async () => {
  ...
  assert.match(source, /getInputContext: \(message\) =>/)
  assert.match(source, /isSigilTextEntryActive\(document, selectionModeCommentEditorEl\)/)
  assert.match(source, /sourceIdentity: message\.inputIdentity/)
  assert.doesNotMatch(source, /normalizeSigilVoiceInputSourceIdentity/)
