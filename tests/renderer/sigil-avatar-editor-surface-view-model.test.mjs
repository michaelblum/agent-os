...
test('compact avatar surface view model maps object graph tabs to toolkit form controls', () => {
  ...
  assert.equal(prismHeight.binding.state_path, 'cylinderHeight');
  ...
  assert.equal(omegaEnabled.binding.state_path, 'isOmegaEnabled');
  ...
  assert.equal(omegaTetartoidB.binding.state_path, 'tetartoidB');
