...
test('Sigil radial camera bundle request carries canonical context session and keyframe', () => {
  ...
  assert.match(requestBlock, /const activeContext = liveJs\.activeContext\?\.context_session/)
  assert.match(requestBlock, /updateActiveContextFromReticle\(reticleContextSession, reason\)/)
  assert.match(requestBlock, /const contextKeyframe = activeContext\.context_keyframe \|\| createContextKeyframeForSession/)
  ...
  assert.match(requestBlock, /context_unavailable: contextSession \? null : \(activeContext\.unavailable \|\| \{[\s\S]*reticle_context_session_unavailable/)
