...
test('AX cursor telemetry emits for same-role elements with different identity evidence', () => {
  ...
  assert.match(signatureBody, /hit\.settableAttributeNames\.joined/)
  assert.match(signatureBody, /hit\.ancestorChain\.map/)
  assert.doesNotMatch(signatureBody, /capabilities/)
  assert.doesNotMatch(signatureBody, /contextPath/)
  assert.match(queryBody, /settable_attributes: hit\.settableAttributeNames/)
  assert.match(queryBody, /ancestor_chain: axAncestorPayloads\(hit\.ancestorChain\)/)
...
test('AX cursor telemetry carries raw browser window and tab context evidence', () => {
  ...
  assert.doesNotMatch(axSource, /axLooksLikeBrowserApp/)
